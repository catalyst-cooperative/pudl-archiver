...
@asynccontextmanager
async def retry_async_contextmanager(
    async_func: Callable[..., Awaitable[Any]],
    args: list | None = None,
    kwargs: dict | None = None,
    retry_count: int = 7,
    ...
    for try_count in range(1, retry_count + 1):
        # try count is 1 indexed for logging clarity
        try:
            async with async_func(*args, **kwargs) as res:
                yield res
                break
        except retry_on as e:
            if try_count == retry_count:
            ...
            retry_delay_s = retry_base_s * 2 ** (try_count - 1)
            logger.info(
                f"Error while executing {async_func.__name__} (try #{try_count}, retry in {retry_delay_s}s): {e}"
            )
            await asyncio.sleep(retry_delay_s)
        finally:
            pass
