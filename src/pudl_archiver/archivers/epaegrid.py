...
class EpaEgridArchiver(AbstractDatasetArchiver):
    ...
    async def get_resources(self) -> ArchiveAwaitable:
        ...
        for link in await self.get_hyperlinks(BASE_URL, link_pattern):
            ...
            yield self.get_year_resource(year, BASE_URL)

        recent_year = max(years) + 1
        recent_url = "https://www.epa.gov/egrid/detailed-data"
        # TODO: Add all of the most recent data that's not on the main page
        # there are two additional webpages that actually have the most recent data
        # https://www.epa.gov/egrid/summary-data
        # https://www.epa.gov/egrid/egrid-technical-guide
        yield self.get_year_resource(recent_year, recent_url)

        # TODO: add the PM files as their own separate zip
        # https://www.epa.gov/egrid/egrid-pm25

    async def get_year_resource(self, year: int, base_url: str) -> ResourceInfo:
        """Download xlsx file."""
        ...
        for link in await self.get_hyperlinks(base_url, table_link_pattern):
            ...
            await self.download_file(base_url, download_path)
            self.add_to_archive(
                zip_path=zip_path,
                filename=filename,
